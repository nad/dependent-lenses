...
module Dependent₃ where
  ...
  x = record
    { to         = P.→-to-⟶ λ r → (R₁.f r , R₁.lemma r) , R₁.x r
    ; from       = P.→-to-⟶ _
    ; inverse-of = record
  ...
  f = record
    { to         = P.→-to-⟶ λ r → R₁.x r , (R₁.f r , R₁.lemma r)
    ; from       = P.→-to-⟶ _
    ; inverse-of = record
      { left-inverse-of  = λ _ → P.refl
      ; right-inverse-of = λ _ → P.refl
  ...
  A : Lens₃ R₂ (Lift {ℓ = suc zero} ⊤) (λ _ → R₂)
  A = id₃

  r₁ : Lens₃ R₂ Set R₁
  r₁ = record
    { to         = P.→-to-⟶ λ r → R₂.A r , R₂.r₁ r
    ; from       = P.→-to-⟶ _
    ; inverse-of = record
      { left-inverse-of  = λ _ → P.refl
      ; right-inverse-of = λ _ → P.refl
